...
Production domain is set in `internal/platform/config/config.go` (`PublicHostSuffix`, `PublicURLScheme`). The CLI dials **`clickly.cv:9000`** for the tunnel; override with `DEVTUNNEL_SERVER=localhost:9000` for local dev.
