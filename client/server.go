...
func startTunneling(protocol, port string) {
	...
	for {
		...
		//wait here to get data
		go handleRequest(conn, tunnelRequest, port)
	}
}

func handleRequest(conn net.Conn, request TunnelRequest, port string) {
	client := &http.Client{}
	localServer := "http://localhost:" + port
	reqUrl, err := http.NewRequest(request.Method, localServer+request.Path, bytes.NewBuffer(request.Body))
	reqUrl.Header = request.Headers
